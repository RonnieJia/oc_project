...
- (CGFloat)heightWithMaxWid:(CGFloat)wid font:(CGFloat)size {
    CGRect rect = [self boundingRectWithSize:CGSizeMake(wid, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFont(size)} context:nil];
    
    return rect.size.height;
}

- (CGFloat)widthWithMaxWid:(CGFloat)wid font:(CGFloat)size {
    CGRect rect = [self boundingRectWithSize:CGSizeMake(wid, 20) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:kFont(size)} context:nil];
    
    return rect.size.width;
}

- (NSString *)priceNum {
    if (IsStringEmptyOrNull(self)) {
        return self;
    }
...
//    }
    ...
    NSArray *compateArr = [self componentsSeparatedByString:@"."];
    if (compateArr.count < 2) {
        return self;
    }
    NSString *sec = compateArr[1];
    if (sec.length < 3) {
        return self;
    }
    return [NSString stringWithFormat:@"%@.%@",[compateArr objectAtIndex:0], [sec substringToIndex:2]];
}
