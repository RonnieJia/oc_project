...
- (UIView *)redView {
    if (!_redView) {
        _redView = RJCreateView(CGRectMake(KScreenWidth-(KScreenWidth/6)+10, 5, 10, 10), [UIColor redColor], YES, 5, nil, 0);
        _redView.hidden = YES;
    }
    return _redView;
}
...
- (instancetype)sharedInit {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showRedView:) name:@"msgShowBadge" object:nil];
    return self;
}

- (void)showRedView:(NSNotification *)noti {
    if ([CurrentUserInfo sharedInstance].showMsg) {
        self.redView.hidden=YES;
        return;
    }
    BOOL show = [noti.object boolValue];
