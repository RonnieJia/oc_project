...
- (void)setSelectedIndex:(NSUInteger)selectedIndex {
    ...
    UIControl *selectedControl = [self.CYLtabBar cyl_tabBarButtonWithTabIndex:selectedIndex];
    if (selectedControl) {
...
- (void)updateSelectionStatusIfNeededForTabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController shouldSelect:(BOOL)shouldSelect {
    ...
    if (!shouldConfigureSelectionStatus) {
        [self.CYLtabBar cyl_stopAnimationOfAllLottieView];
    }
}
...
- (void)didSelectControl:(UIControl *)control {
    ...
    if (shouldSelectViewController) {
        [self.CYLtabBar.cyl_visibleControls enumerateObjectsUsingBlock:^(UIControl * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            obj.selected = NO;
        }];
...
- (void)addLottieImageWithControl:(UIControl *)control animation:(BOOL)animation {
     NSUInteger index = [self.CYLtabBar.cyl_subTabBarButtonsWithoutPlusButton indexOfObject:control];
    if (NSNotFound == index) {
        return;
    }
    ...
    if (animation) {
        [self.CYLtabBar cyl_animationLottieImageWithSelectedControl:control lottieURL:lottieURL size:lottieSize];
    }
}
